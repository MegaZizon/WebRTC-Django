...
function createOfferer(peerUsername, receiver_channel_name){
    //RTCPreer.. 함수의 매개변수는 스턴서버 null 했을시 Local
    ...
    peer.addEventListener('icecandidate', (event) => {
        if(event.candidate){
            console.log('[offer]new ice candidate');
            return;
        }
...
function createAnswers(offer, peerUsername, receiver_channel_name){
    //RCTPreer.. 함수의 매개변수는 스턴서버 null 했을시 Local
    var peer = new RTCPeerConnection(null);
    ...
    peer.addEventListener('icecandidate', (event) => {
        if(event.candidate){
            console.log('[answer]new ice candidate');
            return;
        }
...
function sleep(ms) {
  ...
  async function example() {
    console.log('시작');
    await sleep(2000); // 2초 동안 일시적으로 지연
    console.log('2초 후에 출력됨');
  }